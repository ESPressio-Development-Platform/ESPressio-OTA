...
/**
 * Arduino IDE form of the ESP32 OTA provider-readiness demo.
 *
 * This example is intentionally read-only: it inspects boot/storage facts,
 * performs application-image preflight and exercises SHA-256 without staging
 * bytes, selecting a boot target, marking a trial or restarting the device.
 */
...
public:
    ...
    void Poll() noexcept {
        // A real application can advance its OTA Coordinator from its own
        // cooperative scheduler here; ESPressio-OTA creates no hidden task.
        delay(1000);
    }
};
